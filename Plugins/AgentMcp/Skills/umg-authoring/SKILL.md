...
- **Style tokens.** Name the few colors, radii and font sizes the screens use (panel, accent, text, muted text, success, danger,
  rarity colors) and reuse those exact values everywhere.
...
- **C++ base per component** (a `UUserWidget` subclass, `UCLASS(Abstract)`):
  ...
  - Colors that depend on data (rarity, state) come from the style tokens in C++; shapes, paddings and static colors stay in the
    Widget Blueprint so that designers can still change them.
...
2. Look at it in a play session: `pie_start`, show the screen the way the project does (the Agent MCP testbed has
   `sample_show_widget`), `viewport_capture`, `pie_stop`. Wait for intro animations to finish before capturing.
